...
<view class="container">
  ...
  <view wx:else>
    ...
    <view class="text">{{userInfo.description}}</view>
  </view>
</view>
```
...
Page({
  ...
  async loadMore() {
    ...
    try {
      const newData = await request.get('/api/list', {
        page: this.data.page,
...
if (wx.canIUse('getPrivacySetting')) {
  ...
  console.log('当前版本不支持 getPrivacySetting')
}
...
- [ ] **功能测试**: 所有功能正常运行，无明显 bug
- [ ] **真机预览**: 在 iOS 和 Android 真机上测试通过
- [ ] **代码体积**: 
  - 主包 < 2MB
  - 总包（含分包）< 20MB
  - 单个分包 < 2MB
...
Page({
  ...
  onShareAppMessage(options) {
    ...
    // options.target: 如果是 button，target 是触发分享的 button 元素
...
- `wx.getStorage()` / `wx.getStorageSync()` - 获取数据
- `wx.removeStorage()` / `wx.removeStorageSync()` - 删除数据
- `wx.clearStorage()` / `wx.clearStorageSync()` - 清空缓存
